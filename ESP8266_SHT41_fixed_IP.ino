...
#define RelayPIN        14             // Define the Relay Control pin
#define RelayReverse    false          // Set to true for Relay that requires a signal LOW for ON

#define SensorAddress   0x77           // Use 0x77 for an Adafruit/Sparkfun variant, otherwise also 0x76 may work
...
//################ VARIABLES ################
const char* ssid       = "SSID";             // WiFi SSID     replace with details for your local network
const char* password   = "wifi_password";         // WiFi Password replace with details for your local network
...
void setup() {
  ...
  server.begin();
  StartSensorSHT41();
  ReadSensorSHT41();                                           // Get current sensor values
...
void loop() {
  if ((millis() - LastTimerSwitchCheck) > TimerCheckDuration) {
    ...
    ReadSensorSHT41();                                    // Get sensor readings, or get simulated values if 'simulated' is ON
...
//#########################################################################################
void StartSensorSHT41() {
  //Wire.begin(4, 5);                              // Default SDA and SCL pins
  ...
  if (!simulating) {                               // If not sensor simulating, then start the real one
        ...
        sht4.setPrecision(SHT4X_MED_PRECISION);  // SHT4X_HIGH_PRECISION, SHT4X_MED_PRECISION, SHT4X_LOW_PRECISION: 
        // You can have 6 different heater settings
          //SHT4X_NO_HEATER
          //SHT4X_LOW_HEATER_100MS, SHT4X_LOW_HEATER_1S
          //SHT4X_MED_HEATER_100MS, SHT4X_MED_HEATER_1S
          //SHT4X_HIGH_HEATER_100MS, SHT4X_HIGH_HEATER_1S
